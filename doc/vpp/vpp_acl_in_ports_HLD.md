...
| v0.5 | 09/05/2026 | Longxiang Lyu (lolv@microsoft.com) | Review feedback. Corrected the description of the sticky-failure mechanism: orchagent's retry allocates a fresh VID and leaks the previous object rather than being rejected as a duplicate, so the rule stays in `PENDING_CREATION` and is never reported `ACTIVE`. Recorded that an unresolved port is a lookup miss rather than a down interface, and that nothing re-drives the fan-out when the port later resolves, so a transient miss becomes durable. Added the `MAX_ACL_ATTRS` truncation limitation, which is now warned about |
...
An entry is recorded in `m_objectHash` and `m_acl_tbl_rules_map` by `createAclEntry()` *before* `AclAddRemoveCheck()` runs, and is not rolled back if that check fails. orchagent does not remove a rule whose create failed; it retries, and because `RedisRemoteSaiInterface::create()` allocates a fresh VID on every call, the retry is never recognised as a duplicate — it leaks the previous object into `m_objectHash` and leaves the rule in `PENDING_CREATION`. Failing the entry would therefore leave the table permanently unprogrammable, with an object leaked per retry — a silent failure of exactly the kind this section exists to prevent, and a **regression** for a configuration that programs today, since `IN_PORTS` was previously ignored outright rather than rejected.
...
Failing the whole table was considered and rejected, for the same reasons set out in [Why This Is Ingress-Only](#why-this-is-ingress-only). ACL tables are shared — MuxOrch's drop rule and the PFC watchdog's rules both live in `IngressTableDrop` — so one unresolvable entry would stop unrelated features from programming. The failure would also be sticky: `createAclEntry()` commits the entry to `m_objectHash` and `m_acl_tbl_rules_map` before `AclAddRemoveCheck()` runs and rolls back neither, so every later add or remove on that table would walk the same entry and fail again, leaving the table frozen at its last good rule set while the rule sits in `PENDING_CREATION` and each retry leaks another object.

Partial resolution follows the same rule rather than a separate one: rules are emitted for the ports that resolved, and each failure is logged. Resolving *none* is simply the degenerate case of that. Emitting nothing when only some ports resolve would be strictly worse for a DENY entry — it would drop the scoping on the ports that were resolvable too, so a partial result is closer to the intent than an empty one.

What a partial result must **not** be read as is a statement about the port. An unresolved port is a name-to-`hwif` lookup miss, not evidence that the interface is down: `vpp_get_hwif_name()` resolves through `m_port_id_to_tapname`, which is populated only as host interfaces are created, so a port that is forwarding perfectly well can fail to resolve if the entry is programmed before its hostif exists.

That matters because **nothing re-runs the fan-out when a port later becomes resolvable.** `AclTblConfig()` is reached only from `createAclEntry()`, `removeAclEntry()` and `setAclEntry()` — all ACL mutations on that table. No port or host-interface event triggers it. An entry whose scope resolved partially, or not at all, therefore keeps that scope until some unrelated add or remove on the same table happens to re-walk it. For a standby mux `DROP` this leaves a fail-open window on precisely the port the rule exists to protect.

This is a bounded regression rather than a new one — the previous behaviour ignored `IN_PORTS` entirely, so the same entry was unscoped permanently and on every port — but it is a real gap. Closing it means reprogramming the table when a named port becomes resolvable (or rolling the entry back in `createAclEntry()` so orchagent's retry re-drives it); see [Restrictions and Limitations](#restrictions-and-limitations).
...
- **A resolution miss is not re-driven when the port appears.** `AclTblConfig()` runs only on an ACL mutation of that table, so an entry that resolved to a subset of its ports — or to none — keeps that scope until an unrelated add or remove re-walks it. A transient miss during bring-up therefore becomes a durable one. The fix is to reprogram the table when a named port becomes resolvable, or to roll the entry back in `createAclEntry()` so orchagent's retry re-drives it; both are out of scope here and tracked as follow-up work.
- **An entry with more than `MAX_ACL_ATTRS` attributes is read incompletely.** `get_sorted_aces()` caps the per-entry dump at 12, and the attribute store is keyed by name, so the surviving attributes are an alphabetical cut. A rich `L3V4V6` entry can push both `IN_PORTS` and `PRIORITY` past the cut, which would leave a scoped entry looking unscoped and the rule at priority 0. This predates the feature and is not fixed here; reaching the cap is now logged as a warning.
